...
class NuScenesCustomMetric(BaseMetric):
    ...
    def __init__(self,
        ...
        self.eval_det_only = eval_det_only
    ...
    def process(self, data_batch: dict, data_samples: Sequence[dict]) -> None:
        ...
        for data_sample in data_samples:
            ...
            for attr_name in pred_3d:
                if attr_name != 'eval_det_only':
                    pred_3d[attr_name] = pred_3d[attr_name].to('cpu')
            result['pred_instances_3d'] = pred_3d
            for attr_name in pred_2d:
                pred_2d[attr_name] = pred_2d[attr_name].to('cpu')
            result['pred_instances'] = pred_2d
            result['ret_iou'] = data_sample['ret_iou']
            if isinstance(data_sample['sample_idx'], list):
                data_sample['sample_idx'] = data_sample['sample_idx'][0]
    ...
    def compute_metrics(self, results: List[dict]) -> Dict[str, float]:
        ...
        self.data_infos = sorted(annotations, key=lambda e: e['timestamp']) if not self.eval_det_only else annotations   # tracking 数据在训练加载时候就按照timestamp排序，检测数据不需要排序
    ...
    def nus_evaluate(self,
        ...
        for name in result_dict:
            if name != 'ret_iou':
                ...
                if result_dict[name][0] is not None:
                    print(f'Evaluating ious of pred map seg')
                    import numpy as np
                    inter=[0,0,0]
                    union=[0,0,0]
                    res=[0,0,0]
                    for i in range(len(result_dict[name])):
                        for tt in range(3):
                            inter[tt]+=result_dict[name][i][0][tt]
                            union[tt]+=result_dict[name][i][1][tt]
                    for i in range(len(res)):
                        res[i]=(inter[i]/union[i]).item()
                    print(inter)
                    print(union)
                    print(res)
                    print(len(result_dict[name]))
    ...
    def _evaluate_single(
        ...
        if len(self.data_infos) == 81:
            self.version = 'v1.0-mini'
        eval_set_map = {
            'v1.0-mini': 'mini_val',
            'v1.0-trainval': 'val',
        }
        if self.eval_det_only:
    ...
    def format_results(
        ...
        for name in results[0]:
            if 'pred' in name and '3d' in name and name[0] != '_' and results[0][name]:
                ...
                for i in range(len(results)):
                    ret_iou.append(results[i][name])
                result_dict[name] = ret_iou
    ...
    def _format_lidar_bbox(self,
        ...
        """
        nusc_annos = {}
        if results[0].get('eval_det_only', True):
            mapped_class_names = classes
            print('Using all classes for converting...')
        else:
            mapped_class_names = self.track_classes
            print('Using track classes for converting...')
        ...
        for i, det in enumerate(mmengine.track_iter_progress(results)):
            ...
            for i, box in enumerate(boxes):
                ...
                nusc_anno = dict(
                    ...
                    attribute_name=attr,
                    tracking_name=name,
                    tracking_score=det['track_scores'][i].item() if 'track_scores' in det else 0,
                    tracking_id=str(det['track_ids'][i].item()) if 'track_ids' in det else '')
                annos.append(nusc_anno)
            nusc_annos[sample_token] = annos
        nusc_submissions = {
            'meta': self.modality,

...
def parse_args():
    ...
    parser.add_argument("--fix_yaw", default=True)
    ...
    for i in range(size):
        ...
        for item in outputs:
            ...
            if args.fix_yaw:
                print('fix yaw============================')
                # 1. 旋转四元数
                q = Quaternion(item['rotation'])
                q_rot = Quaternion(axis=[0, 0, 1], radians=np.pi / 2)
                q_new = (q * q_rot).elements.tolist()  # 绕 z 轴顺时针转 90°
                rotation = q_new

                # 2. 旋转速度
                vx, vy = item['velocity']
                rotated_vx = vx * 0.0 - vy * 1.0
                rotated_vy = vx * 1.0 + vy * 0.0
                velocity = [rotated_vx, rotated_vy]
...
if __name__ == '__main__':
    # TODO(Itachi): fix yaw bug
    # v0.x -> v1.0+   pred_yaw = pred_yaw_v0x - np.pi / 2
    # v1.0+ -> v0.x   pred_yaw = pred_yaw_v1.0+ + np.pi / 2
    save_first_frame()
    main()
    # test_time()
    eval_tracking()
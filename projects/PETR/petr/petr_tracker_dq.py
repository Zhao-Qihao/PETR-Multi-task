...
from projects.PETR.core.track_dq_utils import FFN, QueryInteractionX
from projects.PETR.core.track.instances import Instances as Tracklet
from nuscenes.eval.detection.config import config_factory

@MODELS.register_module()
class PETRTrackerDQ(MVXTwoStageDetector):
    """PETRTrackerDQ."""
    def __init__(self,
                 use_grid_mask=False,
        ...
        super(PETRTrackerDQ,
              self).__init__(pts_voxel_layer,
                             ...
                             pts_bbox_head, pts_seg_head, img_roi_head, img_rpn_head,
                             train_cfg, test_cfg,
                              data_preprocessor)
        ...
        self.loss_ = TASK_UTILS.build(loss_cfg)
    ...
    def tracklet_asso_test(self, tracker, detector, time_delta=None, img_metas=None):
        ...
        tracker, match_pairs, valid_pair = self.tracklet_update_test(   # NOTE：not used in export to pt2
                                                            tracker, 
                                                            detector, 
                                                            track_embedding_nb, 
                                                            time_delta,
    ...
    def sort_and_filter(self, detector, img_metas):
        ...
        l2e_mat = img_metas[0]['lidar2ego'][0].to(pred_cls.device)
    ...
    def forward_single(self, img, detector, img_metas, depth_map=None, frame_idx=0):
        ...
        if self.training:
            ...
            loss_det, match_info = self.loss_.loss_det(det_dict, frame_idx)    
    ...
    def loss(self, inputs=None, data_samples=None):
        """Calls either forward_train or forward_test depending on whether
        return_loss=True.
        Note this setting will change the expected inputs. When
        `return_loss=True`, img and img_metas are single-nested (i.e.
        ...
        # img_tensors = [item for item in kwargs['inputs']['img']]
        img_tensors = [item for item in inputs['img']]
        img = torch.stack(img_tensors, dim=0)
        ...
        instance_inds = [ds.metainfo['instance_inds'] for ds in data_samples]
        num_samples = len(data_samples)
        num_frames = len(data_samples[0].metainfo['timestamp'])
        ...
        for frame_idx in range(num_frames):
            ...
            for sample_idx in range(num_samples):
                frame_timestamps.append(data_samples[sample_idx].metainfo['timestamp'][frame_idx])
            lidar_timestamp.append(torch.stack(frame_timestamps).to(device))
        # move to GPU
        gt_bboxes_3d = [[frame.to(device) for frame in batch] for batch in gt_bboxes_3d]
        gt_labels_3d = [[frame.to(device) for frame in batch] for batch in gt_labels_3d]
        instance_inds = [[frame.to(device) for frame in batch] for batch in instance_inds]

        return self.forward_train(img_metas=img_metas, 
    ...
    def forward_train(self,
        ...
        for batch_idx in range(batch_num):
            ...
            timestamp_single = [_time[batch_idx] for _time in lidar_timestamp]
            # init gt instances!
            loss_dict_single = {}
            gt_tracklet_list = []
            ...
            self.loss_.clip_init(gt_tracklet_list)
            self.timestamp = None
            for frame_idx in range(num_frame):
                img_single = img[batch_idx][frame_idx][None]
                img_metas_single = [deepcopy(img_metas[batch_idx])]
                img_metas_single = self.add_lidar2img(img_metas_single)
                lidar2global = img_metas_single[0]['lidar2global'][frame_idx]
                img_metas_single[0]['l2g_r_mat'] = torch.tensor(lidar2global[:3, :3])  # 提取旋转矩阵
                img_metas_single[0]['l2g_t'] = torch.tensor(lidar2global[:3, 3])        # 提取平移向量
                ...
                img_metas_single[0]['gt_bboxes_3d'] = gt_bboxes_3d[batch_idx][frame_idx]
                img_metas_single[0]['gt_labels_3d'] = gt_labels_3d[batch_idx][frame_idx]
                ...
                loss_track = self.loss_.loss_track(tracker, 
                                                det2track_mat, 
                                                info_single, 
    ...
    def predict(self, inputs=None, data_samples=None, mode=None, **kwargs):
        ...
        img = torch.stack(img_tensors, dim=0)
        img_metas = [ds.metainfo for ds in data_samples]
        # gt_bboxes_3d = [ds.gt_instances_3d['gt_bboxes_3d'] for ds in data_samples]
        # gt_labels_3d = [ds.gt_instances_3d['gt_labels_3d'] for ds in data_samples]
        instance_inds = [ds.metainfo['instance_inds'] for ds in data_samples]
        num_samples = len(data_samples)
        num_frames = len(data_samples[0].metainfo['timestamp'])
        ...
        for frame_idx in range(num_frame):
            ...
            img_metas_single[0]['lidar2img'] = img_metas_single[0]['lidar2img'][frame_idx]
            # img_metas_single[0]['box_type_3d'] = img_metas_single[0]['box_type_3d'][frame_idx]
            # img_metas_single[0]['gt_bboxes_3d'] = gt_bboxes_3d[0][frame_idx]
            # img_metas_single[0]['gt_labels_3d'] = gt_labels_3d[0][frame_idx]
            ...
            if self.tracker_cfg.get('eval_det_only', False):
                frame_res = self.tracklet2results(detector)
            else:
                tracker = self.tracklet_asso_test(tracker,
                                                  detector,
                                                  time_delta,
                                                  img_metas_single)
                frame_res = self.tracklet2results(tracker)
            results.append(frame_res)
        
        self.test_detector = detector    # NOTE: 这里把detector传递到下一帧的必要性？
        self.test_tracker = tracker

        results_list_3d = results
        for i, data_sample in enumerate(data_samples):
            results_list_3d_i = InstanceData(
                metainfo=results_list_3d[i])
            data_sample.pred_instances_3d = results_list_3d_i
            data_sample.pred_instances = InstanceData()
            data_sample.ret_iou = None
    ...
    def tracklet2results(self, tracker):
        '''
        Outs:
        ...
        bboxes_dict = self.bbox_coder.decode(bbox_dict, eval_det_only)[0]
        ...
        result_dict = dict(
            ...
            eval_det_only=self.tracker_cfg.get('eval_det_only', False)
        )
    ...
    def add_lidar2img(self, batch_input_metas):
        ...
        for meta in batch_input_metas:
            ...
            for frame_idx in range(len(meta['cam2img'])):
                lidar2img_rts_f = []
                for i in range(len(meta['cam2img'][0])):
                    lidar2cam_rt = torch.tensor(meta['lidar2cam'][frame_idx][i]).double()
                    intrinsic = torch.tensor(meta['cam2img'][frame_idx][i]).double()
                    ...
                    lidar2img_rts_f.append(lidar2img_rt)
                lidar2img_rts.append(lidar2img_rts_f)

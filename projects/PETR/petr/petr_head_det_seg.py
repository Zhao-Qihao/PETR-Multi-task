...
class PETRHead_det_seg(AnchorFreeHead):
    ...
    def __init__(self,
        ...
        assert num_feats * 2 == self.embed_dims, 'embed_dims should' \
            ...
            f' and {num_feats}.'
        self.act_cfg = transformer_lane.get('act_cfg',
        ...
        super(PETRHead_det_seg, self).__init__(
        ...
        self.loss_veh = MODELS.build(loss_veh)
        
        self.positional_encoding = TASK_UTILS.build(positional_encoding)
        
        self.transformer_lane = MODELS.build(transformer_lane)
    ...
    def _init_layers(self):
        ...
        self.reference_points = nn.Embedding(self.num_lane, 3)
    ...
    def _load_from_state_dict(self, state_dict, prefix, local_metadata, strict,
        ...
        if (version is None or version < 2) and self.__class__ is PETRHead_det_seg:
            convert_dict = {
    ...
    def forward(self, mlvl_feats, img_metas):
        ...
        # TODO(Itachi): fix bug here: query_embeds not used
        # outs_dec, _ = self.transformer_lane(x, masks, query_lane, pos_embed)
        outs_dec, _ = self.transformer_lane(x, masks, query_embeds, pos_embed)
        outs_dec = torch.nan_to_num(outs_dec)
        lane_queries=outs_dec
        # combined_query = torch.cat([query_lane, query_embeds], dim=0)
        # outs_dec, _ = self.transformer_lane(x, masks, combined_query, pos_embed)
        # lane_queries = outs_dec[:, :, :self.num_lane]     # [nb_dec, bs, num_lane, embed_dim]
        # obj_queries = outs_dec[:, :, self.num_lane:]      # [nb_dec, bs, num_obj, embed_dim]
        ...
        for lvl in range(outs_dec.shape[0]):
            # NOTE(Itachi): original: lane_queries_lvl = lane_queries[lvl].reshape(1,25,25,-1).permute(0,3,1,2).contiguous(). fix bug here
            lane_queries_lvl=lane_queries[lvl].reshape(batch_size, int(math.sqrt(self.num_lane)), int(math.sqrt(self.num_lane)), -1).permute(0,3,1,2).contiguous()
        ...
        for lvl in range(outs_dec.shape[0]):
            ...
            outputs_class = self.cls_branches[lvl](obj_queries[lvl]).to(
                torch.float32)
            tmp = self.reg_branches[lvl](obj_queries[lvl]).to(torch.float32)
    ...
    def loss_seg_single(self,
        ...
        gt_lane_list = torch.stack([gt for gt in gt_lane_list]).contiguous()
        gt_lane_list = gt_lane_list.view(3,-1,1)
        # 将gt_lane_list中的batch维度展开


        loss_dri=self.loss_dri(lane_preds[0],gt_lane_list[0].long())
        loss_lan=self.loss_lan(lane_preds[1],gt_lane_list[1].long())
        loss_veh=self.loss_veh(lane_preds[2],gt_lane_list[2].long())
    ...
    def loss_seg(self,
        ...
        losses_dri, losses_lan, losses_veh = multi_apply(
            self.loss_seg_single, all_lane_preds,all_gt_lanes_list)
        loss_dict = dict()
    ...
    def loss_det(self,
        ...
        losses_cls, losses_bbox = multi_apply(self.loss_det_single,
    ...
    def loss_by_feat(self,
        ...
        loss_dict = dict()
        # preds_dicts
        # {'all_lane_preds': tensor([[[[ 9.4066e-...ackward0>), 'all_cls_scores': tensor([[[[-3.6234, ...ackward0>), 'all_bbox_preds': tensor([[[[-3.2629e+...pySlices>), 'enc_cls_scores': None, 'enc_bbox_preds': None}
        preds_dicts_seg = {'all_lane_preds': preds_dicts['all_lane_preds']}
        preds_dicts_det = {k: preds_dicts[k] for k in ['all_cls_scores', 'all_bbox_preds', 'enc_cls_scores', 'enc_bbox_preds']}
        loss_seg = self.loss_seg(gt_bboxes_list, gt_labels_list, preds_dicts_seg, maps, gt_bboxes_ignore)
        loss_det = self.loss_det(gt_bboxes_list, gt_labels_list, preds_dicts_det, gt_bboxes_ignore)

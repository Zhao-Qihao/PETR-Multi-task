...
class PETRHead_seg(AnchorFreeHead):
    ...
    def _init_layers(self):
        ...
        self.reference_points_lane =torch.cat([xy[0].reshape(-1)[...,None],xy[1].reshape(-1)[...,None]],-1)
    ...
    def forward(self, mlvl_feats, img_metas):
        ...
        for img_id in range(batch_size):
            for cam_id in range(num_cams):
                if isinstance(img_metas[img_id]['img_shape'], tuple):
        ...
        reference_points_lane = self.reference_points_lane.to(x.device)
        query_lane=self.query_embedding_lane(pos2posemb2d(reference_points_lane))
        ...
        for lvl in range(outs_dec_lane.shape[0]):
            ...
            lane_queries_lvl=lane_queries[lvl].reshape(batch_size,25,25,-1).permute(0,3,1,2).contiguous()

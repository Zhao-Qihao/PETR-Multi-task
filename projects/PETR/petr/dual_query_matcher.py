...
from projects.PETR.core.track.instances import Instances
from projects.PETR.core.bbox.util import normalize_bbox
...
class DualQueryMatcher(nn.Module):
    def __init__(self, 
        ...
        self.loss_asso = MODELS.build(loss_asso)
        ...
        if loss_me is not None:
            self.loss_me = TASK_UTILS.build(loss_me)
        else:
            self.loss_me = None
        ...
        if loss_depth is not None:
            if 'BCELoss' in loss_depth.type:
                ...
                self.loss_trans = MODELS.build(loss_depth)
        else:
    ...
    def _get_target_single(self,
        ...
        # DETR
        if len(pos_inds ) != 0 and len(sampling_result.pos_gt_bboxes) !=0 :

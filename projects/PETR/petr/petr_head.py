...
class PETRHead(AnchorFreeHead):
    ...
    def position_embeding(self, img_feats, img_metas, masks=None):
        ...
        img2lidars = coords.new_tensor(img2lidars, device=coords.device)  # (B, N, 4, 4)
    ...
    def forward(self, mlvl_feats, img_metas):
        ...
        # NOTE(Itachi): pos_embed是对输入特征图中的每个像素点进行编码,用于表示图像中每个像素的位置。由position_embeding得到
        # query_embeds是由pos2posemb3d对reference_points进行位置编码(cos/sin编码)得到的，目的是提供模型要检测的目标的初始 3D 坐标位置，指导模型关注哪些区域。
        # 虽然reference_points一开始是nn.Embedding(self.num_query, 3)得到的，但是在训练过程中，reference_points 会被不断优化，逐渐逼近真实目标的位置。
        outs_dec, _ = self.transformer(x, masks, query_embeds, pos_embed,
                                       self.reg_branches)
        # NOTE(Itachi): outs_dec shape: (num_decoder_layers, B, num_query, embed_dim)
